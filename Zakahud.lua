...
--==============================================================================--
--            KĨ NĂNG 2: CƯỠI RỒNG LỬA HỒI SINH (ĐÃ SỬA CHẤT LIỆU BẠN/TRONG MỜ)  --
--==============================================================================--
local function SetFireDragonMount(state)
    Settings.FireDragonMount = state
    local char = LocalPlayer.Character
    ...
    if state then
        ...
        local function createPart(name, size, color, mat, trans)
            ...
            p.Size = size
            p.Color = color or Color3.fromRGB(180, 0, 0)
            -- Đổi sang Glass hoặc SmoothPlastic kết hợp Transparency để có chiều sâu, không bị đặc như nhựa cứng
            p.Material = mat or Enum.Material.Glass 
            p.Transparency = trans or 0.2
            p.CanCollide = false
            p.Massless = true
            p.Parent = dragonFolder
            return p
        ...
        local bodyPart = createPart("DragonBody", Vector3.new(4, 3.5, 8), Color3.fromRGB(160, 0, 0), Enum.Material.Glass, 0.15)
        local bodyWeld = attachWeld(root, bodyPart, CFrame.new(0, -3.8, 0))

        for i = -3, 3 do
            local spine = createPart("BodySpine", Vector3.new(0.5, 1.5, 0.8), Color3.fromRGB(255, 100, 0), Enum.Material.Neon, 0)
            attachWeld(bodyPart, spine, CFrame.new(0, 2.2, i * 1.1) * CFrame.Angles(math.rad(-20), 0, 0))
        end

        local headPart = createPart("DragonHead", Vector3.new(3.2, 2.8, 4.5), Color3.fromRGB(220, 20, 0), Enum.Material.Glass, 0.1)
        local headWeld = attachWeld(bodyPart, headPart, CFrame.new(0, 1.2, -5.5))

        local snout = createPart("DragonSnout", Vector3.new(2.6, 1.8, 3), Color3.fromRGB(180, 0, 0), Enum.Material.Glass, 0.1)
        attachWeld(headPart, snout, CFrame.new(0, -0.3, -2.5))

        local leftEye = createPart("LeftEye", Vector3.new(0.4, 0.4, 0.8), Color3.fromRGB(255, 255, 0), Enum.Material.Neon, 0)
        attachWeld(headPart, leftEye, CFrame.new(-1.3, 0.7, -1.2))

        local rightEye = createPart("RightEye", Vector3.new(0.4, 0.4, 0.8), Color3.fromRGB(255, 255, 0), Enum.Material.Neon, 0)
        attachWeld(headPart, rightEye, CFrame.new(1.3, 0.7, -1.2))

        local leftHorn = createPart("LeftHorn", Vector3.new(0.6, 2.8, 0.6), Color3.fromRGB(255, 140, 0), Enum.Material.Neon, 0)
        attachWeld(headPart, leftHorn, CFrame.new(-1.2, 1.8, 1.2) * CFrame.Angles(math.rad(-35), math.rad(-20), 0))

        local rightHorn = createPart("RightHorn", Vector3.new(0.6, 2.8, 0.6), Color3.fromRGB(255, 140, 0), Enum.Material.Neon, 0)
        attachWeld(headPart, rightHorn, CFrame.new(1.2, 1.8, 1.2) * CFrame.Angles(math.rad(-35), math.rad(20), 0))
        ...
        local tail1 = createPart("DragonTail1", Vector3.new(2.8, 2.5, 6), Color3.fromRGB(140, 0, 0), Enum.Material.Glass, 0.2)
        local tail1Weld = attachWeld(bodyPart, tail1, CFrame.new(0, -0.4, 6))

        local tail2 = createPart("DragonTail2", Vector3.new(2.0, 1.8, 6), Color3.fromRGB(180, 30, 0), Enum.Material.Glass, 0.2)
        local tail2Weld = attachWeld(tail1, tail2, CFrame.new(0, 0, 5.5))

        local tailTip = createPart("TailTip", Vector3.new(0.8, 2.5, 2.5), Color3.fromRGB(255, 60, 0), Enum.Material.Neon, 0)
        attachWeld(tail2, tailTip, CFrame.new(0, 0, 3.2) * CFrame.Angles(math.rad(45), 0, 0))

        local function makeLeg(legName, pos)
            local leg = createPart(legName, Vector3.new(1.2, 3.2, 1.2), Color3.fromRGB(130, 0, 0), Enum.Material.Glass, 0.2)
            attachWeld(bodyPart, leg, pos)

            local claw = createPart(legName .. "Claw", Vector3.new(1.4, 0.5, 1.5), Color3.fromRGB(255, 200, 0), Enum.Material.Neon, 0)
            attachWeld(leg, claw, CFrame.new(0, -1.5, -0.3))
        end
        makeLeg("FrontLeftLeg", CFrame.new(-2.2, -1.8, -2.5))
        makeLeg("FrontRightLeg", CFrame.new(2.2, -1.8, -2.5))
        ...
        local leftWingBase = createPart("LeftWingBase", Vector3.new(1.2, 1.2, 5), Color3.fromRGB(200, 0, 0), Enum.Material.Glass, 0.2)
        local leftWingWeld = attachWeld(bodyPart, leftWingBase, CFrame.new(-2.2, 1.5, -1))

        local leftWingBlade = createPart("LeftWingBlade", Vector3.new(10, 0.2, 6), Color3.fromRGB(255, 50, 0), Enum.Material.Glass, 0.35) -- Mờ ảo như cánh năng lượng
        attachWeld(leftWingBase, leftWingBlade, CFrame.new(-5, 0, 1) * CFrame.Angles(0, math.rad(-15), 0))

        local rightWingBase = createPart("RightWingBase", Vector3.new(1.2, 1.2, 5), Color3.fromRGB(200, 0, 0), Enum.Material.Glass, 0.2)
        local rightWingWeld = attachWeld(bodyPart, rightWingBase, CFrame.new(2.2, 1.5, -1))

        local rightWingBlade = createPart("RightWingBlade", Vector3.new(10, 0.2, 6), Color3.fromRGB(255, 50, 0), Enum.Material.Glass, 0.35)
        attachWeld(rightWingBase, rightWingBlade, CFrame.new(5, 0, 1) * CFrame.Angles(0, math.rad(15), 0))
...
--==============================================================================--
--        KĨ NĂNG 3: CÁNH THIÊN THẦN & KHIÊN CẦU VỒNG (ĐÃ TĂNG ĐỘ TRONG MỜ)       --
--==============================================================================--
local function SetRainbowAngel(state)
    Settings.RainbowAngel = state
    local char = LocalPlayer.Character
    ...
    if state then
        ...
        shield.Shape = Enum.PartType.Ball
        shield.Size = Vector3.new(10, 10, 10)
        shield.Material = Enum.Material.Glass -- Dùng Glass để tạo hiệu ứng trong suốt bóng bẩy
        shield.Transparency = 0.45 -- Làm trong suốt, không bị đặc như nhựa
        shield.Color = Color3.fromRGB(0, 255, 255)
        shield.CanCollide = false
        shield.Massless = true
        shield.Parent = angelFolder
        ...
        hl.Name = "ShieldHighlight"
        hl.Adornee = shield
        hl.FillTransparency = 0.75 -- Tăng độ trong suốt của viền sáng
        ...
        local function createWing(sideName)
            ...
            wingMain.Name = "WingMain"
            wingMain.Size = Vector3.new(0.3, 7.5, 3.5)
            wingMain.Material = Enum.Material.Glass
            wingMain.Transparency = 0.25 -- Cánh có độ trong suốt mờ ảo đẹp mắt
            wingMain.Color = Color3.fromRGB(255, 255, 255)
            wingMain.CanCollide = false
            wingMain.Massless = true
            wingMain.Parent = wingModel
            ...
            for i = 1, 5 do
                local feather = Instance.new("Part")
                feather.Size = Vector3.new(0.2, 4.5 - (i * 0.5), 1.8)
                feather.Material = Enum.Material.Glass
...
--==============================================================================--
--        KĨ NĂNG 4: LỒNG BĂNG NHỐT (ĐÃ CHỈNH ĐỘ TRONG SUỐT BĂNG KỲ DIỆU)        --
--==============================================================================--
local function CreateIceCageSuper()
    local myRoot = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not myRoot then return end
    ...
    if targetPlayer and targetPlayer.Character then
        ...
        local function makeWall(cf, s)
            ...
            wall.CFrame = cf
            wall.Material = Enum.Material.Ice
            wall.Color = Color3.fromRGB(150, 240, 255)
            wall.Transparency = 0.4 -- Tăng độ trong suốt để nhìn thấu bên trong, giống băng thật thay vì nhựa đục
            wall.Reflectance = 0.3 -- Tạo độ bóng lóa mắt cho băng
            wall.Anchored = true
            wall.CanCollide = true
            wall.Parent = cageModel
        end
