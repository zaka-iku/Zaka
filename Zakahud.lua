...
--==============================================================================--
--[[
    ZAKA HUD v3 - COMPLETE NEW VERSION
    Menu đẹp + Nhiều Tab + Superman Fly + Magic
    Tối ưu Delta (Mobile + PC)
]]
...
local Settings = {
    ...
    ESPHealth = true, ESPDistance = true, ESPTracer = false,
    ESPMaxDist = 2500, ESPTeamCheck = false,

    -- Player
    Speed = false, SpeedValue = 28,
    Fly = false, FlySpeed = 75,
    Noclip = false, InfiniteJump = false,
    ClickTP = false, BunnyHop = false, GodMode = false,

    -- Visuals
    Fullbright = false, NoFog = false, HeadLight = false,
    RainbowBody = false,

    -- Magic
    FireAura = false, IceAura = false, ElectricAura = false,
    RainbowTrail = false, AngelWings = false,

    -- Misc
    SpinBot = false, Fling = false, Invisible = false,
    AntiAFK = true,
}
...
local NoclipConn, SpeedConn, HitboxConn, GodConn, RainbowConn, SpinConn
...
local function CreateCape(char)
    ...
    CapeConn = RunService.RenderStepped:Connect(function()
        ...
        if Flying then
            local sway = math.sin(tick() * 9) * 0.18
            weld.C0 = base * CFrame.Angles(math.rad(-18), sway, 0)
        else
            local sway = math.sin(tick() * 2.2) * 0.07
            weld.C0 = base * CFrame.Angles(math.rad(4), sway, 0)
        end
    end)
end
...
RunService.RenderStepped:Connect(function()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        t.Name.Position = Vector2.new(pos.X, pos.Y - size.Y/2 - 15)
        ...
        t.Tracer.To = Vector2.new(pos.X, pos.Y + size.Y/2)
        t.Tracer.Visible = Settings.ESPTracer
    end
end)
...
local function GetMagicFolder()
    local char = LocalPlayer.Character
    if not char then return end
    local folder = char:FindFirstChild("ZakaMagic")
    if not folder then
        folder = Instance.new("Folder")
        folder.Name = "ZakaMagic"
        folder.Parent = char
    end
    return folder
end
...
local function SetFireAura(v)
    ...
    fire.Size = 7
    fire.Heat = 11
    fire.Color = Color3.fromRGB(255,110,20)
...
local function SetIceAura(v)
    ...
    local part = Instance.new("Part")
    part.Name = "IceAura"
    part.Size = Vector3.new(5.5, 0.25, 5.5)
    part.Color = Color3.fromRGB(140, 220, 255)
...
OpenBtn.BackgroundColor3 = Color3.fromRGB(0, 145, 255)
OpenBtn.Text = "Z"
...
Main.Position = UDim2.new(0, 70, 0.22, 0)
...
for i, name in ipairs(Tabs) do
    ...
    btn.BackgroundColor3 = name == Current and Color3.fromRGB(0, 160, 255) or Color3.fromRGB(30, 30, 40)
    btn.Text = name
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        for n, b in pairs(TabBtns) do
            b.BackgroundColor3 = n == name and Color3.fromRGB(0, 160, 255) or Color3.fromRGB(30, 30, 40)
        end
...
-- Combat Tab
AddToggle(Pages["Combat"], "Aimbot", false, function(v) Settings.Aimbot = v end)
...
-- ESP Tab
AddToggle(Pages["ESP"], "ESP Enabled", false, function(v) Settings.ESP = v end)
AddToggle(Pages["ESP"], "Box", true, function(v) Settings.ESPBox = v end)
AddToggle(Pages["ESP"], "Name", true, function(v) Settings.ESPName = v end)
AddToggle(Pages["ESP"], "Health", true, function(v) Settings.ESPHealth = v end)
AddToggle(Pages["ESP"], "Distance", true, function(v) Settings.ESPDistance = v end)
AddToggle(Pages["ESP"], "Tracer", false, function(v) Settings.ESPTracer = v end)
...
print("Zaka HUD v3 Loaded - New Complete Version")
local Settings = {
    -- Combat & Hitbox
    Aimbot = false,
    AimbotFOV = 120,
