...
namespace AdvancedGameSystem
{
    #region ENUMS & DATA STRUCTURES
    public enum ElementType { Physical, Fire, Ice, Lightning, Dark, Earth }
    public enum ComboRank { D, C, B, A, S, SS, SSS }

    [System.Serializable]
    public class MagicSpellData
    {
        ...
        public GameObject projectilePrefab; // Fallback tự động nếu null
        public GameObject magicCircleVFX;
        public GameObject impactVFX;
        public AudioClip castSFX, releaseSFX, impactSFX;
    }
    ...
    public class ComboNode
    {
        ...
        public bool launchesEnemy = false;
    }
    #endregion

    #region OBJECT POOLING SYSTEM (TỐI ƯU HIỆU NĂNG PARTICLE)
    public class FXObjectPool : MonoBehaviour
    {
        public static FXObjectPool Instance { get; private set; }
        ...
        public GameObject SpawnFX(GameObject prefab, Vector3 position, Quaternion rotation, float autoDespawnTime = 2f)
        {
            ...
            GameObject obj;
            if (poolDict[key].Count > 0 && !poolDict[key].Peek().activeSelf)
            {
            ...
            StartCoroutine(DespawnRoutine(key, obj, autoDespawnTime));
            return obj;
        }

        private IEnumerator DespawnRoutine(string key, GameObject obj, float delay)
        {
    ...
    #region MAIN COMBAT & MAGIC CONTROLLER
    [RequireComponent(typeof(CharacterController))]
    [RequireComponent(typeof(AudioSource))]
    public class AdvancedCombatController : MonoBehaviour
    {
        ...
        public List<ComboNode> meleeComboChain;

        [Header("--- MAGIC SPELLS DATA ---")]
        public List<MagicSpellData> spellList;
        public GameObject defaultParticleFallback; // Tự gán nếu chiêu thiếu FX
        ...
        #region 1. CHÍNH SỬA & HIỆU ỨNG MAGIC (SPELL FX SYSTEM)
        public void CastMagicSpell(int spellIndex)
        {
            if (spellIndex >= spellList.Count || isCasting) return;
            MagicSpellData spell = spellList[spellIndex];

            // Kiểm tra Cooldown
            if (cooldownTimers.ContainsKey(spell.spellID) && cooldownTimers[spell.spellID] > 0)
                return;

            if (mana < spell.manaCost) return;
            mana -= spell.manaCost;

            StartCoroutine(SpellCastRoutine(spell));
        }

        private IEnumerator SpellCastRoutine(MagicSpellData spell)
        {
            isCasting = true;
            cooldownTimers[spell.spellID] = spell.cooldown;

            // 1. Hiệu ứng Vòng Ma Pháp (Magic Circle Overlay)
            GameObject circleFX = FXObjectPool.Instance.SpawnFX(spell.magicCircleVFX, transform.position, Quaternion.identity, spell.castTime);
            PlaySFX(spell.castSFX);

            yield return new WaitForSeconds(spell.castTime);

            // 2. Tự động kiểm tra Fallback FX nếu thiếu Visual
            GameObject projPrefab = spell.projectilePrefab != null ? spell.projectilePrefab : defaultParticleFallback;

            // 3. Khởi tạo đường đạn/chiêu thức
            if (projPrefab != null)
            {
                Vector3 spawnPos = transform.position + transform.forward * 1.5f + Vector3.up * 1.2f;
                GameObject proj = FXObjectPool.Instance.SpawnFX(projPrefab, spawnPos, transform.rotation, 5f);
                PlaySFX(spell.releaseSFX);

                MagicProjectile projectileScript = proj.GetComponent<MagicProjectile>();
                if (projectileScript == null) projectileScript = proj.AddComponent<MagicProjectile>();
                
                projectileScript.Setup(spell, targetEnemy, transform);
            }
        ...
        #region 2. COMBAT ENGINE (PARRY, HITSTOP, COMBO, AIR JUGGLE)
        public void ExecuteMeleeAttack()
        {
            if (isCasting) return;

            if (Time.time - lastAttackTime > 1.5f) currentComboIndex = 0;

            if (currentComboIndex < meleeComboChain.Count)
            {
                ComboNode node = meleeComboChain[currentComboIndex];
                
                // Soft Auto-Targeting: Xoay về đối thủ gần nhất
                if (targetEnemy != null) transform.LookAt(targetEnemy.position);

                // Hitbox detection
                Collider[] hits = Physics.OverlapSphere(transform.position + transform.forward, 2f, enemyLayer);
                foreach (var hit in hits)
                {
                    EnemyHealth enemy = hit.GetComponent<EnemyHealth>();
                    if (enemy != null)
                    {
                        float damage = 10f * node.damageMultiplier;
                        enemy.TakeDamage(damage, spellList.Count > 0 ? spellList[0].element : ElementType.Physical);

                        // Trigger Hitstop (Khựng đòn) & Screen Shake
                        CombatUtility.Instance.TriggerHitstop(0.08f);
                        CombatUtility.Instance.TriggerScreenShake(0.2f, 0.5f);

                        // Hất tung hoặc Đẩy lùi (Knockback)
                        if (node.launchesEnemy) enemy.ApplyAirLaunch(node.knockbackForce);
                        else enemy.ApplyKnockback(transform.forward, node.knockbackForce);
                    }
                }

                lastAttackTime = Time.time;
                currentComboIndex = (currentComboIndex + 1) % meleeComboChain.Count;
            }
        }

        // Cơ chế Parry (Đỡ đòn chính xác trong 0.2s)
        public void TriggerParry()
        {
            isParrying = true;
            parryWindowTimer = 0.2f;
        }
        ...
        // Perfect Dodge & Invincibility Dash Frames
        public void PerformPerfectDodge(Vector3 dodgeDirection)
        {
        ...
        private IEnumerator DodgeRoutine(Vector3 dir)
        {
            ...
            {
                controller.Move(dir * 12f * Time.deltaTime);
    ...
    #region MAGIC PROJECTILE (TÊN PHÉP & TƯƠNG TÁC)
    public class MagicProjectile : MonoBehaviour
    {
        private MagicSpellData spellData;
        private Transform target;
        private Transform caster;
        private float speed = 20f;

        public void Setup(MagicSpellData data, Transform targetEnemy, Transform casterTransform)
        {
            spellData = data;
            target = targetEnemy;
        ...
        private void Update()
        {
            ...
            // Homing Physics
            if (spellData.isHoming && target != null)
            {
            ...
            foreach (var hit in hits)
            {
                if (hit.transform != caster && !hit.isTrigger)
                {
                    OnImpact(hit);
                    break;
                }
        ...
        private void OnImpact(Collider hit)
        {
            // Hiệu ứng nổ & Thiệt hại
            FXObjectPool.Instance.SpawnFX(spellData.impactVFX, transform.position, Quaternion.identity);

            EnemyHealth enemy = hit.GetComponent<EnemyHealth>();
            if (enemy != null)
            {
                enemy.TakeDamage(spellData.baseDamage, spellData.element);
            }
    ...
    #region AI KẺ ĐỊCH, KHÁNG CHIÊU & REACTION
    public class EnemyHealth : MonoBehaviour
    {
        public float maxHealth = 200f;
        public float currentHealth;
        public float stunGauge = 100f;
        public ElementType currentStatusElement = ElementType.Physical;
        ...
        public void TakeDamage(float amount, ElementType element)
        {
            ...
            // Tương tác phản ứng Hệ (Elemental Reaction Combo)
            ApplyElementalReaction(element);

            // Break / Stun System
            stunGauge -= amount * 0.5f;
            if (stunGauge <= 0)
            {
                // Trạng thái gục ngã (Break State)
                amount *= 1.5f; // Chịu thêm 150% sát thương
            }

            if (currentHealth <= 0) Die();
        }

        private void ApplyElementalReaction(ElementType incoming)
        {
            if (currentStatusElement == ElementType.Fire && incoming == ElementType.Lightning)
            {
                // Phản ứng Nổ Siêu Dẫn
                TakeDamage(30f, ElementType.Physical);
                CombatUtility.Instance.TriggerScreenShake(0.3f, 1f);
            }
            currentStatusElement = incoming;
        }
        ...
        public void ApplyAirLaunch(float force)
        {
    ...
    #region COMBAT UTILITIES (HITSTOP & SCREEN SHAKE)
    public class CombatUtility : MonoBehaviour
    {
        public static CombatUtility Instance { get; private set; }
        ...
        // Rung màn hình (Dynamic Screen Shake)
