...
local ZenitsuCool1, ZenitsuCool2, ZenitsuCool3 = false, false, false
local ZenitsuSword = nil
local ZenitsuBubble = nil

local function ZenitsuLightning(pos, n)
    n = n or 5
    for i = 1, n do
        local p = Instance.new("Part")
...
local function CreateZenitsuSword()
    local char = LocalPlayer.Character
    if not char then return end
    if ZenitsuSword then ZenitsuSword:Destroy() end
...
local function CreateZenitsuBubble()
    local char = LocalPlayer.Character
    if not char then return end
...
local function ToggleZenitsu(state)
    ZenitsuOn = state
    local char = LocalPlayer.Character
    if not char then return end
    ...
    if state then
        ...
        if ZenitsuSword then ZenitsuSword:Destroy() ZenitsuSword = nil end
        if ZenitsuBubble then ZenitsuBubble:Destroy() ZenitsuBubble = nil end
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then
            hum.WalkSpeed = 16
            hum.JumpPower = 50
...
local function ZenitsuSkill1()
    if ZenitsuCool1 or not ZenitsuOn then return end
    ...
    for i = 1, 8 do
        ...
        ZenitsuLightning(root.Position, 3)
        task.wait(0.04)
    end
    task.delay(2.5, function() ZenitsuCool1 = false end)
end

local function ZenitsuSkill2()
    if ZenitsuCool2 or not ZenitsuOn then return end
    local root = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not root then return end
    ZenitsuCool2 = true
    ...
    for i = 1, 6 do
        ...
        ZenitsuLightning(root.Position, 6)
        task.wait(0.07)
    end
    TweenService:Create(Camera, TweenInfo.new(0.3), {FieldOfView = old}):Play()
    task.delay(3.2, function() ZenitsuCool2 = false end)
end

local function ZenitsuSkill3()
    if ZenitsuCool3 or not ZenitsuOn then return end
    local root = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not root then return end
    ZenitsuCool3 = true
    local old = Camera.FieldOfView
    ...
    for i = 1, 7 do
        ...
        ZenitsuLightning(root.Position, 5)
        task.wait(0.045)
    end
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr \~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if (t.Position - root.Position).Magnitude < 13 then
                pcall(function() t.AssemblyLinearVelocity = Vector3.new(0, 90, 0) end)
                ZenitsuLightning(t.Position, 7)
            end
        end
    end
    task.wait(0.25)
    ZenitsuLightning(root.Position, 9)
    TweenService:Create(Camera, TweenInfo.new(0.35), {FieldOfView = old}):Play()
    task.delay(4, function() ZenitsuCool3 = false end)
end
--==============================================================================--
...
CreateToggle(Pages[6], "Zenitsu Sword (Vĩnh Viễn + Ngủ)", false, function(v) ToggleZenitsu(v) end)
CreateButton(Pages[6], "Zenitsu Skill 1: Lướt Ngủ", function() ZenitsuSkill1() end)
CreateButton(Pages[6], "Zenitsu Skill 2: Lướt 6 Lần + Sét", function() ZenitsuSkill2() end)
CreateButton(Pages[6], "Zenitsu Skill 3: Lao + Hất + Đóng Kiếm", function() ZenitsuSkill3() end)
